...
func TestAction_RepoRepair(t *testing.T) {
	// setup test server
	...
	for _, test := range tests {
		err := repoRepair(cli.NewContext(&cli.App{Name: "vela", Version: "v0.0.0"}, test.set, nil))

		if test.failure {
			if err == nil {
				t.Errorf("repoRepair should have returned err")
			}
		...
		if err != nil {
			t.Errorf("repoRepair returned err: %v", err)
		}
	}
}
