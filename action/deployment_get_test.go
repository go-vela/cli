...
func TestAction_DeploymentGet(t *testing.T) {
	// setup test server
	...
	for _, test := range tests {
		err := deploymentGet(cli.NewContext(&cli.App{Name: "vela", Version: "v0.0.0"}, test.set, nil))

		if test.failure {
			if err == nil {
				t.Errorf("deploymentGet should have returned err")
			}
		...
		if err != nil {
			t.Errorf("deploymentGet returned err: %v", err)
		}
	}
}
