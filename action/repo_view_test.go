...
func TestAction_RepoView(t *testing.T) {
	// setup test server
	...
	for _, test := range tests {
		err := repoView(cli.NewContext(&cli.App{Name: "vela", Version: "v0.0.0"}, test.set, nil))

		if test.failure {
			if err == nil {
				t.Errorf("repoView should have returned err")
			}
		...
		if err != nil {
			t.Errorf("repoView returned err: %v", err)
		}
	}
}
