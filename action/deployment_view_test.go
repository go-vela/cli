...
func TestAction_DeploymentView(t *testing.T) {
	// setup test server
	...
	for _, test := range tests {
		err := deploymentView(cli.NewContext(&cli.App{Name: "vela", Version: "v0.0.0"}, test.set, nil))

		if test.failure {
			if err == nil {
				t.Errorf("deploymentView should have returned err")
			}
		...
		if err != nil {
			t.Errorf("deploymentView returned err: %v", err)
		}
	}
}
