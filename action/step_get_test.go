...
func TestAction_StepGet(t *testing.T) {
	// setup test server
	...
	for _, test := range tests {
		err := stepGet(cli.NewContext(&cli.App{Name: "vela", Version: "v0.0.0"}, test.set, nil))

		if test.failure {
			if err == nil {
				t.Errorf("stepGet should have returned err")
			}
		...
		if err != nil {
			t.Errorf("stepGet returned err: %v", err)
		}
	}
}
