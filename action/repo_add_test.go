...
func TestAction_RepoAdd(t *testing.T) {
	// setup test server
	...
	for _, test := range tests {
		err := repoAdd(cli.NewContext(&cli.App{Name: "vela", Version: "v0.0.0"}, test.set, nil))

		if test.failure {
			if err == nil {
				t.Errorf("repoAdd should have returned err")
			}
		...
		if err != nil {
			t.Errorf("repoAdd returned err: %v", err)
		}
	}
}
