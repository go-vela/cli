...
func TestAction_HookGet(t *testing.T) {
	// setup test server
	...
	for _, test := range tests {
		err := hookGet(cli.NewContext(&cli.App{Name: "vela", Version: "v0.0.0"}, test.set, nil))

		if test.failure {
			if err == nil {
				t.Errorf("hookGet should have returned err")
			}
		...
		if err != nil {
			t.Errorf("hookGet returned err: %v", err)
		}
	}
}
