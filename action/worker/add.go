...
func (c *Config) Add(client *vela.Client) error {
	...
	// convert WorkerRegistration struct into bytes for request body
	b, err := json.Marshal(wr)
	if err != nil {
		return fmt.Errorf("unable to marshal worker registration details: %w", err)
	}
	...
	req, err := http.NewRequestWithContext(context.Background(), "POST", workerRegistrationURL, body)
	if err != nil {
