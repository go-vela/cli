...
func TestAction_LogGet(t *testing.T) {
	// setup test server
	...
	for _, test := range tests {
		err := logGet(cli.NewContext(&cli.App{Name: "vela", Version: "v0.0.0"}, test.set, nil))

		if test.failure {
			if err == nil {
				t.Errorf("logGet should have returned err")
			}
		...
		if err != nil {
			t.Errorf("logGet returned err: %v", err)
		}
	}
}
