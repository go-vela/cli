...
func TestAction_SecretRemove(t *testing.T) {
	// setup test server
	...
	for _, test := range tests {
		err := secretRemove(cli.NewContext(&cli.App{Name: "vela", Version: "v0.0.0"}, test.set, nil))

		if test.failure {
			if err == nil {
				t.Errorf("secretRemove should have returned err")
			}
		...
		if err != nil {
			t.Errorf("secretRemove returned err: %v", err)
		}
	}
}
