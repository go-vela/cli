...
func TestAction_HookView(t *testing.T) {
	// setup test server
	...
	for _, test := range tests {
		err := hookView(cli.NewContext(&cli.App{Name: "vela", Version: "v0.0.0"}, test.set, nil))

		if test.failure {
			if err == nil {
				t.Errorf("hookView should have returned err")
			}
		...
		if err != nil {
			t.Errorf("hookView returned err: %v", err)
		}
	}
}
