...
func TestAction_RepoRemove(t *testing.T) {
	// setup test server
	...
	for _, test := range tests {
		err := repoRemove(cli.NewContext(&cli.App{Name: "vela", Version: "v0.0.0"}, test.set, nil))

		if test.failure {
			if err == nil {
				t.Errorf("repoRemove should have returned err")
			}
		...
		if err != nil {
			t.Errorf("repoRemove returned err: %v", err)
		}
	}
}
