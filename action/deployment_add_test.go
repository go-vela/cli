...
func TestAction_DeploymentAdd(t *testing.T) {
	// setup test server
	...
	for _, test := range tests {
		err := deploymentAdd(cli.NewContext(&cli.App{Name: "vela", Version: "v0.0.0"}, test.set, nil))

		if test.failure {
			if err == nil {
				t.Errorf("deploymentAdd should have returned err")
			}
		...
		if err != nil {
			t.Errorf("deploymentAdd returned err: %v", err)
		}
	}
}
